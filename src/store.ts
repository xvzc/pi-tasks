 ...
 *   execution note. `assignment: null` removes that field. Successful updates refresh `updatedAt` and never touch
...
export function sanitizeSessionId(sessionId: string): string {
  const readable =
    sessionId.replace(/[^A-Za-z0-9_-]/g, "-").slice(0, 80) || "session";
...
function assertNoPrefix(value: Record<string, unknown>): void {
  if ("prefix" in value) {
    throw new TaskError("`prefix` is no longer supported.");
  }
}

const ASSIGNMENT_KEYS = new Set(["delegate", "owner"]);

function parseAssignment(raw: unknown, label = "assignment"): Assignment {
  if (!isRecord(raw)) throw new TaskError(`${label} must be an object.`);
  assertOnlyKeys(raw, ASSIGNMENT_KEYS, label);
  if (raw.delegate === false && raw.owner === null)
    return { delegate: false, owner: null };
  if (
    raw.delegate === true &&
    typeof raw.owner === "string" &&
    raw.owner.trim().length > 0
  ) {
    return { delegate: true, owner: raw.owner };
  }
  throw new TaskError(
    `${label} must be { delegate: false, owner: null } or { delegate: true, owner: string }.`,
  );
}
...
function assertNoColor(value: Record<string, unknown>): void {
  if ("color" in value) {
    throw new TaskError(
      "`color` is no longer supported; glyph colors are fixed by task status.",
    );
  }
}
...
function assertOnlyKeys(
  ...
  if (unknown.length > 0)
    throw new TaskError(`${label} contains unknown key: ${unknown[0]}.`);
}
...
function parseTaskLog(raw: unknown): TaskLogEntry[] {
  ...
  if (!Array.isArray(raw))
    throw new TaskError("Persisted task log must be an array.");
  return raw.map((entry) => {
    ...
    if (typeof entry === "string") {
      ...
      if (!isUtcTimestamp(timestamp) || message.length === 0) {
        throw new TaskError(
          "Persisted task log entries must contain a valid timestamp and non-empty message.",
        );
      }
...
function parseTask(raw: unknown): Task {
  if (!isRecord(raw))
    throw new TaskError("Each persisted task must be an object.");
  ...
  if (
    ...
    throw new TaskError(
      "Persisted task maxAttempts must be a non-negative safe integer.",
    );
  }
  if (
    ...
    throw new TaskError(
      "Persisted task attempt must be a non-negative safe integer.",
    );
  }
  if (
    (raw.maxAttempts as number) > 0 &&
    (raw.attempt as number) > (raw.maxAttempts as number)
  ) {
  ...
  if (typeof raw.description !== "string")
    throw new TaskError("Persisted task description must be a string.");
  ...
  if (!isTaskStatus(persistedStatus))
    throw new TaskError(
      `Invalid persisted task status: ${String(raw.status)}.`,
    );
  ...
  if (!Array.isArray(raw.blockedBy))
    throw new TaskError(
      "Persisted task blockedBy must be an array of task ids.",
    );
  assertIdList(raw.blockedBy as number[]);
  const reviewOf = raw.reviewOf ?? [];
  if (!Array.isArray(reviewOf))
    throw new TaskError(
      "Persisted task reviewOf must be an array of task ids.",
    );
  assertIdList(reviewOf as number[], "reviewOf");
  if (!isRecord(raw.metadata))
    throw new TaskError("Persisted task metadata must be an object.");
  const log = parseTaskLog(raw.log);
  if (!isUtcTimestamp(raw.createdAt) || !isUtcTimestamp(raw.updatedAt)) {
    throw new TaskError(
      "Persisted task timestamps must be valid ISO 8601 UTC strings.",
    );
  }
  const assignment =
    "assignment" in raw
      ? parseAssignment(raw.assignment, "Persisted task assignment")
      : undefined;
  ...
  if ("startedAt" in raw && raw.startedAt !== undefined) {
    if (!isUtcTimestamp(raw.startedAt)) {
      throw new TaskError(
        "Persisted task startedAt must be a valid ISO 8601 UTC string.",
      );
    }
  ...
  if ("tookMs" in raw && raw.tookMs !== undefined) {
    if (
      typeof raw.tookMs !== "number" ||
      !Number.isSafeInteger(raw.tookMs) ||
      raw.tookMs < 0
    ) {
      throw new TaskError(
        "Persisted task tookMs must be a non-negative safe integer.",
      );
    }
...
function parseHistoryCycle(raw: unknown): TaskHistoryCycle {
  if (!isRecord(raw))
    throw new TaskError("Persisted task history cycles must be objects.");
  assertOnlyKeys(raw, HISTORY_CYCLE_KEYS, "Persisted task history cycle");
  if (!isUtcTimestamp(raw.archivedAt)) {
    throw new TaskError(
      "Persisted task history archivedAt must be a valid ISO 8601 UTC string.",
    );
  }
  if (!Array.isArray(raw.tasks) || raw.tasks.length === 0) {
    throw new TaskError(
      "Persisted task history cycle tasks must be a non-empty array.",
    );
  }
  if (
    typeof raw.totalActiveMs !== "number" ||
    !Number.isSafeInteger(raw.totalActiveMs) ||
    raw.totalActiveMs < 0
  ) {
    throw new TaskError(
      "Persisted task history totalActiveMs must be a non-negative safe integer.",
    );
  }
  ...
  for (const entry of raw.tasks) {
    ...
    if (!isArchivableStatus(task.status)) {
      throw new TaskError(
        "Persisted task history may contain only completed or deleted tasks.",
      );
    }
    if (tasks.has(task.id))
      throw new TaskError(`Duplicate persisted history task id: #${task.id}.`);
...
export class TaskStore {
  ...
  constructor(
    ...
    private readonly writer: TaskStoreWriter = writeStoreData,
  ) {}
  ...
  static async load(
    filePath: string,
    writer: TaskStoreWriter = writeStoreData,
  ): Promise<TaskStore> {
    ...
    try {
      ...
      if (!isRecord(data))
        throw new TaskError("Persisted task store must be an object.");
      if (data.version !== 1 && data.version !== 2) {
        throw new TaskError(
          `Unsupported persisted task store version: ${String(data.version)}.`,
        );
      }
      assertOnlyKeys(
        data,
        data.version === 1 ? STORE_KEYS_V1 : STORE_KEYS_V2,
        "Persisted task store",
      );
      if (!Array.isArray(data.tasks))
        throw new TaskError("Persisted task store tasks must be an array.");
      ...
      if (
        ...
        throw new TaskError(
          "Persisted task store nextId must be a positive safe integer.",
        );
      }
      ...
      for (const entry of data.tasks) {
        const task = parseTask(entry);
        if (tasks.has(task.id))
          throw new TaskError(`Duplicate persisted task id: #${task.id}.`);
        tasks.set(task.id, task);
      }
      const history =
        data.version === 2
          ? (data.history as unknown[]).map(parseHistoryCycle)
          : [];
      ...
      for (const cycle of history) {
        for (const task of cycle.tasks) {
          if (allIds.has(task.id))
            throw new TaskError(
              `Duplicate persisted task id across history: #${task.id}.`,
            );
          allIds.add(task.id);
        }
      }
      for (const task of tasks.values()) {
        if (allIds.has(task.id))
          throw new TaskError(
            `Duplicate persisted task id across active tasks and history: #${task.id}.`,
          );
        allIds.add(task.id);
      }
      ...
      if (data.nextId <= maxId) {
        throw new TaskError(
          `Persisted task store nextId must be greater than every task id.`,
        );
      }
      validateDependencies(tasks);
      checkReviewUniqueness(tasks);
      ...
      if ("totalActiveMs" in data && data.totalActiveMs !== undefined) {
        if (
          typeof data.totalActiveMs !== "number" ||
          !Number.isSafeInteger(data.totalActiveMs) ||
          data.totalActiveMs < 0
        ) {
          throw new TaskError(
            "Persisted task store totalActiveMs must be a non-negative safe integer.",
          );
        }
      ...
      if ("activeSince" in data && data.activeSince !== undefined) {
        if (!isUtcTimestamp(data.activeSince)) {
          throw new TaskError(
            "Persisted task store activeSince must be a valid ISO 8601 UTC string.",
          );
        }
      ...
      throw new TaskError(
        `Invalid persisted task store: ${error instanceof Error ? error.message : String(error)}.`,
      );
    }
  }
  ...
  async createMany(
    ...
    const validated = inputs.map((input, index) =>
      validateCreateBatchItem(input, configuredMaxAttempts, index),
    );
    const refToIndex = new Map<string, number>();
    for (const item of validated) {
      if (item.ref === undefined) continue;
      if (refToIndex.has(item.ref))
        throw new TaskError(`Duplicate task ref: ${item.ref}.`);
    ...
    for (const item of validated) {
      for (const ref of [...item.blockedByRefs, ...item.reviewOfRefs]) {
        ...
        if (dependencyIndex === undefined)
          throw new TaskError(`Unknown blockedByRef: ${ref}.`);
        if (dependencyIndex === item.index)
          throw new TaskError(`Task ref ${ref} cannot depend on itself.`);
      }
    }
    ...
    const archivesTerminal =
      this.tasks.size > 0 &&
      [...this.tasks.values()].every((task) => isArchivableStatus(task.status));
    const candidateTasks = archivesTerminal
      ? new Map<number, Task>()
      : cloneTasks(this.tasks);
    const firstId = this.nextId;
    if (inputs.length > Number.MAX_SAFE_INTEGER - firstId) {
      throw new TaskError(
        "Task ID space is exhausted; no further tasks can be created.",
      );
    }
    const now = nowIso();
    const candidateHistory = archivesTerminal
      ? [
          ...
          historyCycle(this.tasks, this.totalActiveMs, now),
        ]
      : this.history;
    ...
    for (const item of validated) {
      ...
      for (const depId of [...item.blockedBy, ...item.reviewOf]) {
        if (depId === id)
          throw new TaskError(`Task #${id} cannot depend on itself.`);
        if (!candidateTasks.has(depId))
    ...
    for (const index of order) {
      ...
      const localDependencies = item.blockedByRefs.map(
        (ref) => indexToId.get(refToIndex.get(ref) as number) as number,
      );
      const localWriters = item.reviewOfRefs.map(
        (ref) => indexToId.get(refToIndex.get(ref) as number) as number,
      );
      const task: Task = {
        id,
    ...
    const candidateActiveSince = archivesTerminal
      ? undefined
      : this.activeSince;
    await this.writeData(
      candidateTasks,
      candidateNextId,
      candidateTotalActiveMs,
      candidateActiveSince,
      candidateHistory,
    );
    ...
    return {
      created: created.map((mapping) => ({ ...mapping })),
      tasks: createdTasks.map(cloneTask),
    };
  }
  ...
  async updateMany(updates: TaskUpdateBatchInput[]): Promise<Task[]> {
    ...
    for (const update of updates) {
      if (!isRecord(update))
        throw new TaskError("Each update must be an object.");
      assertValidId(update.id);
      if (seen.has(update.id))
        throw new TaskError(`Duplicate task update id: #${update.id}.`);
      seen.add(update.id);
    }
    ...
    for (const update of ordered) {
      ...
      if ("attempt" in update) {
        throw new TaskError(
          "attempt cannot be updated; it increments only on entry into in_progress.",
        );
      }
      if ("maxAttempts" in update) {
        throw new TaskError(
          "maxAttempts cannot be updated; it is set once at creation.",
        );
      }
      ...
      if (update.description !== undefined) {
        if (typeof update.description !== "string")
          throw new TaskError("description must be a string.");
        task.description = update.description;
      }
      if (update.assignment !== undefined) {
        if (update.assignment === null) delete task.assignment;
        else task.assignment = parseAssignment(update.assignment);
      }
      ...
      if (update.metadata !== undefined) {
        if (!isRecord(update.metadata))
          throw new TaskError("metadata must be an object.");
        task.metadata = { ...task.metadata, ...update.metadata };
      }
      if (
        update.appendLog !== undefined &&
        (typeof update.appendLog !== "string" ||
          update.appendLog.trim().length === 0)
      ) {
        throw new TaskError("appendLog must be a non-empty string.");
      }
      if (update.status !== undefined) {
        if (!isTaskStatus(update.status))
          throw new TaskError(`Invalid status: ${String(update.status)}.`);
        ...
        .filter(
          (candidate) =>
            ...
            (candidate.blockedBy.includes(task.id) ||
              candidate.reviewOf.includes(task.id)),
        )
        .map((candidate) => `#${candidate.id}`)
        .sort();
      if (referencers.length > 0) {
        throw new TaskError(
          `Task #${task.id} cannot be deleted: referenced by ${referencers.join(", ")}.`,
        );
      }
    }
    ...
    validateDependencies(candidateTasks);
    checkReviewUniqueness(candidateTasks);
    const candidateTiming = advanceGlobalTiming(
      this.tasks,
      candidateTasks,
      this.totalActiveMs,
      this.activeSince,
  ...
  async archiveTerminalCycle(): Promise<boolean> {
    if (
      this.tasks.size === 0 ||
      [...this.tasks.values()].some((task) => !isArchivableStatus(task.status))
    )
      return false;
    ...
    while (changed) {
      ...
      for (const id of [...retained]) {
        ...
        for (const writerId of reviewer.reviewOf) {
          ...
          if (writer?.status !== "completed" || retained.has(writerId))
            continue;
...
function validateCreateBatchItem(
  ...
  if (!isRecord(input))
    throw new TaskError(`tasks[${index}] must be an object.`);
  assertNoPrefix(input);
  assertNoColor(input);
  if (input.ref !== undefined)
    assertValidTaskRef(input.ref, `tasks[${index}].ref`);
  ...
  const assignment =
    input.assignment === undefined
      ? undefined
      : parseAssignment(input.assignment);
  const blockedBy = input.blockedBy ?? [];
  assertIdList(blockedBy);
  const blockedByRefs = input.blockedByRefs ?? [];
  if (!Array.isArray(blockedByRefs))
    throw new TaskError("blockedByRefs must be an array of task refs.");
  blockedByRefs.forEach((ref, refIndex) =>
    assertValidTaskRef(ref, `tasks[${index}].blockedByRefs[${refIndex}]`),
  );
  const reviewOf = input.reviewOf ?? [];
  assertIdList(reviewOf, "reviewOf");
  const reviewOfRefs = input.reviewOfRefs ?? [];
  if (!Array.isArray(reviewOfRefs))
    throw new TaskError("reviewOfRefs must be an array of task refs.");
  reviewOfRefs.forEach((ref, refIndex) =>
    assertValidTaskRef(ref, `tasks[${index}].reviewOfRefs[${refIndex}]`),
  );
...
function stableTopologicalOrder(
  ...
  for (const item of items) {
    const uniqueDependencies = new Set(
      [...item.blockedByRefs, ...item.reviewOfRefs].map(
        (ref) => refToIndex.get(ref) as number,
      ),
    );
    indegree[item.index] = uniqueDependencies.size;
    for (const dependencyIndex of uniqueDependencies)
      dependents[dependencyIndex].add(item.index);
  }
  const ready = items
    .filter((item) => indegree[item.index] === 0)
    .map((item) => item.index);
  const order: number[] = [];
  while (ready.length > 0) {
    ...
    for (const dependentIndex of [...dependents[index]].sort((a, b) => a - b)) {
      ...
      if (indegree[dependentIndex] === 0) {
        const insertion = ready.findIndex(
          (readyIndex) => readyIndex > dependentIndex,
        );
  ...
  if (order.length !== items.length)
    throw new TaskError("Batch-local task refs contain a dependency cycle.");
  return order;
}
  ...
  for (const reviewer of tasks.values()) {
    ...
    for (const writerId of new Set(reviewer.reviewOf)) {
      ...
      if (existing !== undefined && existing !== reviewer.id) {
        throw new TaskError(
          `Task #${writerId} is already reviewed by #${existing}.`,
        );
      }
...
export function transitionEligibility(
  ...
  if (current.status === "deleted" && proposed.status !== "deleted") {
    return {
      allowed: false,
      error: `Task #${current.id} is deleted and cannot transition to another status.`,
    };
  }
  if (current.status !== proposed.status) {
    if (
      !(ALLOWED_TRANSITIONS[current.status] as readonly TaskStatus[]).includes(
        proposed.status,
      )
    ) {
      return {
        allowed: false,
        error: `Task #${current.id} cannot transition from ${current.status} to ${proposed.status}.`,
      };
    }
    if (appendLog === undefined || appendLog.trim().length === 0) {
      return {
        allowed: false,
        error: `Transition to ${proposed.status} requires a non-empty appendLog.`,
      };
    }
  }
  ...
  for (const depId of prerequisites) {
    if (depId === proposed.id) {
      return {
        allowed: false,
        error: `Task #${proposed.id} cannot depend on itself.`,
      };
    }
  ...
  if (proposed.status === "in_progress") {
    const open = prerequisites.filter(
      (depId) => proposedTasks.get(depId)?.status !== "completed",
    );
    if (open.length > 0) {
      return {
        allowed: false,
...
function validateDependencies(tasks: Map<number, Task>): void {
  ...
  const visit = (id: number): void => {
    if (visiting.has(id))
      throw new TaskError(`Task #${id} is part of a dependency cycle.`);
...
function assertValidMaxAttempts(value: unknown): asserts value is number {
  if (
    ...
    throw new TaskError(
      `Invalid maxAttempts: ${String(value)}. Expected a positive safe integer.`,
    );
  }
}
...
function assertValidConfigMaxAttempts(value: unknown): asserts value is number {
  if (
    ...
    throw new TaskError(
      `Invalid maxAttempts: ${String(value)}. Expected a non-negative safe integer (0 means unlimited).`,
    );
  }
}
...
function assertValidId(id: number): void {
  if (typeof id !== "number" || !Number.isSafeInteger(id) || id <= 0) {
    throw new TaskError(
      `Invalid task id: ${String(id)}. Expected a positive safe integer.`,
    );
  }
}
...
function assertIdList(ids: number[], label = "blockedBy"): void {
  if (!Array.isArray(ids))
    throw new TaskError(`${label} must be an array of task ids.`);
