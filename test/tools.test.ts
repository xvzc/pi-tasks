...
interface Captured {
  ...
  beforeAgentStart: ((event: any) => { systemPrompt: string } | undefined) | undefined;
}

function capture(): Captured & { pi: any } {
  const captured: Captured = { tools: new Map(), turnStart: undefined, beforeAgentStart: undefined };
  const pi = {
...
describe("tool registration", () => {
  ...
  it("keeps workflow policy out of tool descriptions and field contracts on parameter schemas", () => {
    const { tools, pi } = capture();
    registerExtension(pi);

    const create = tools.get("task_create");
    const update = tools.get("task_update");
    for (const tool of tools.values()) expect(tool.promptGuidelines).toBeUndefined();
    expect(create.description).not.toContain("complete initial task set");
    expect(update.description).not.toContain("shallow-merges");
    expect(update.description).not.toContain("null to remove");
    const createItem = create.parameters.properties.tasks.items;
    const updateItem = update.parameters.properties.updates.items;
    expect(updateItem.properties.metadata.description).toContain("shallow-merge");
    ...
    expect(updateItem.properties.appendLog.description).toContain("Required for every real status transition");
    expect(createItem.properties.reviewOf.items.minimum).toBe(1);
    expect(createItem.properties.reviewOfRefs.items.pattern).toBe("^[a-z][a-z0-9_-]*$");
    expect(updateItem.properties.reviewOf.description).toContain("Replacement");
    expect(createItem.properties.metadata.description).not.toContain("agentType");
  });

  it("exposes assignee input contracts only when enableAssignee is true", async () => {
    const agentDir = process.env.PI_CODING_AGENT_DIR as string;
    await mkdir(join(agentDir, "extensions"), { recursive: true });
    await writeFile(join(agentDir, "extensions", "pi-tasks.json"), JSON.stringify({ enableAssignee: true }));
    const { tools, pi } = capture();
    registerExtension(pi);
...
describe("tool behavior", () => {
  it("creates reviewOfRefs and replaces reviewOf through tool contracts", async () => {
    ...
    expect(JSON.parse(created.content[0].text).tasks.map((task: { id: number; reviewOf: number[] }) => [task.id, task.reviewOf])).toEqual([
      [1, []],
      [2, [1]],
    ]);
  ...
  it("renders filtered completed rows with full reverse-review context", async () => {
    ...
    const listed = await tools.get("task_list").execute(
      "filtered-list",
      { status: "completed" },
      undefined,
      undefined,
    ...
    expect(JSON.parse(listed.content[0].text).map((task: { id: number }) => task.id)).toEqual([1]);
    expect(listed.details.rendering.expanded).toContain("○ #1 Write → completed");
  });
  ...
  it("preserves widget insertion order while mounted task data changes", async () => {
    ...
    await tools.get("task_update").execute("c3", { updates: [{ id: 1, status: "deleted", appendLog: "note" }] }, undefined, undefined, ctx);
  });
  ...
  it("creates, gets, lists, updates, and soft-deletes through the tools", async () => {
    ...
    const created = await tools.get("task_create").execute("c1", { tasks: [{ subject: "Work", description: "do things" }] }, undefined, onUpdate, ctx);
    expect(created.isError).toBeUndefined();
    expect(widgets).toHaveLength(1);
    expect(JSON.parse(created.content[0].text).tasks[0]).toMatchObject({ id: 1, subject: "Work", status: "pending" });
    expect(created.content[0].text).not.toContain("Created task");
    expect(created.details.rendering).toEqual({ collapsed: "✓ Created 1 task", expanded: "✓ Created 1 task\n  ◌ #1 Work" });

    const listed = await tools.get("task_list").execute("c2", {}, undefined, onUpdate, ctx);
    expect(JSON.parse(listed.content[0].text)).toHaveLength(1);
    expect(listed.details.rendering.collapsed).toBe("✓ Listed 1 task");

    const gotten = await tools.get("task_get").execute("c3", { id: 1 }, undefined, onUpdate, ctx);
    expect(JSON.parse(gotten.content[0].text).subject).toBe("Work");
    expect(gotten.details.rendering.collapsed).toBe("✓ Retrieved task #1");

    await tools.get("task_update").execute("c3b", { updates: [{ id: 1, status: "in_progress", appendLog: "start" }] }, undefined, onUpdate, ctx);
    const updated = await tools.get("task_update").execute("c4", { updates: [{ id: 1, status: "completed", appendLog: "done" }] }, undefined, onUpdate, ctx);
    expect(updated.isError).toBeUndefined();
    expect(JSON.parse(updated.content[0].text).updated[0]).toMatchObject({ id: 1, status: "completed" });
    ...
    const filtered = await tools.get("task_list").execute("c5", { status: "completed" }, undefined, undefined, ctx);
    expect(JSON.parse(filtered.content[0].text)).toHaveLength(1);

    await tools.get("task_update").execute("c5b", { updates: [{ id: 1, status: "in_progress", appendLog: "rework" }] }, undefined, onUpdate, ctx);
    await tools.get("task_update").execute("c5c", { updates: [{ id: 1, status: "paused", appendLog: "pause" }] }, undefined, onUpdate, ctx);
    const deleted = await tools.get("task_update").execute(
      "c6",
      { updates: [{ id: 1, status: "deleted", appendLog: "removed from scope" }] },
      undefined,
      onUpdate,
      ctx,
    );
    expect(deleted.isError).toBeUndefined();
    expect(JSON.parse(deleted.content[0].text).updated[0]).toMatchObject({ id: 1, status: "deleted" });
    expect(deleted.details.rendering).toEqual({
      collapsed: "✓ Updated 1 task",
      expanded: "✓ Updated 1 task\n  ⌫ #1 Work → status, log" });
    const deletedList = await tools.get("task_list").execute("c7", { status: "deleted" }, undefined, undefined, ctx);
  ...
  it("returns clear error results for invalid and not-found operations", async () => {
    ...
    const missing = await tools.get("task_get").execute("c1", { id: 5 }, undefined, undefined, ctx);
    expect(missing.isError).toBe(true);
    expect(missing.content[0].text).toContain("does not exist");

    const invalid = await tools.get("task_update").execute(
      "c2",
    ...
    const badDep = await tools.get("task_create").execute(
      "c3",
    ...
    const gated = await tools.get("task_create").execute("c4", { tasks: [{ subject: "dep", description: "" }] }, undefined, undefined, ctx);
    expect(gated.isError).toBeUndefined();
    const blocked = await tools.get("task_create").execute(
      "c5",
    ...
    const early = await tools.get("task_update").execute("c6", { updates: [{ id: 2, status: "in_progress", appendLog: "note" }] }, undefined, undefined, ctx);
  ...
  it("requires description, omits color, and constrains every ID schema to safe integers", () => {
    ...
    expect(createItem.properties.blockedBy.items.maximum).toBe(Number.MAX_SAFE_INTEGER);
    ...
    expect(updateItem.properties.blockedBy.items.maximum).toBe(Number.MAX_SAFE_INTEGER);
    expect(tools.get("task_get").parameters.properties.id.maximum).toBe(Number.MAX_SAFE_INTEGER);
    expect(updateItem.properties.status.anyOf.map((entry: { const: string }) => entry.const)).toContain("deleted");
  });
  ...
  it("returns full created and post-update task payloads", async () => {
    ...
    const created = await tools
      .get("task_create")
      .execute("c1", { tasks: [{ subject: "Work", description: "do things" }] }, undefined, undefined, ctx);
    expect(created.isError).toBeUndefined();
    ...
    const updated = await tools
      .get("task_update")
      .execute("c2", { updates: [{ id: 1, subject: "Renamed" }] }, undefined, undefined, ctx);
    expect(updated.isError).toBeUndefined();
    ...
    const gotten = await tools.get("task_get").execute("c3", { id: 1 }, undefined, undefined, ctx);
  ...
  it("returns actual post-update tasks and removes nullable fields", async () => {
    ...
    await tools
      .get("task_create")
      .execute("c1", { tasks: [{ subject: "Work", description: "original" }] }, undefined, undefined, ctx);
    const updated = await tools
      .get("task_update")
      .execute("c2", { updates: [{ id: 1, subject: "Renamed", assignee: null }] }, undefined, undefined, ctx);
    expect(updated.isError).toBeUndefined();
    const postUpdate = JSON.parse(updated.content[0].text).updated[0];
    expect(postUpdate).toMatchObject({ id: 1, subject: "Renamed", description: "original" });
    expect("assignee" in postUpdate).toBe(false);
  ...
  it("rejects color supplied to task_create and task_update", async () => {
    ...
    const create = await tools
      .get("task_create")
      .execute("c1", { tasks: [{ subject: "Labeled", description: "details", color: "red" }] }, undefined, undefined, ctx);
    expect(create.isError).toBe(true);
    expect(create.content[0].text).toContain("`color` is no longer supported");

    await tools
      .get("task_create")
      .execute("c2", { tasks: [{ subject: "Labeled", description: "details" }] }, undefined, undefined, ctx);
    const update = await tools
      .get("task_update")
      .execute("c3", { updates: [{ id: 1, color: null }] }, undefined, undefined, ctx);
  ...
  it("clears an all-completed cycle on turn_start and preserves the next task ID", async () => {
    ...
    await tools.get("task_update").execute("c1b", { updates: [{ id: 1, status: "in_progress", appendLog: "start" }] }, undefined, undefined, ctx);
    await tools.get("task_update").execute("c2", { updates: [{ id: 1, status: "completed", appendLog: "done" }] }, undefined, undefined, ctx);
    ...
    const recreated = await tools.get("task_create").execute("c4", { tasks: [{ subject: "b", description: "" }] }, undefined, undefined, ctx);
    expect(recreated.isError).toBeUndefined();
    expect(JSON.parse(recreated.content[0].text).tasks[0]).toMatchObject({ id: 2, subject: "b" });
  });
  ...
  it("preserves the completed file and widget when a reset task_create fails validation", async () => {
    ...
    const badSubject = await tools
      .get("task_create")
      .execute("c3", { tasks: [{ subject: "  ", description: "" }] }, undefined, undefined, ctx);
    ...
    const staleDep = await tools
      .get("task_create")
      .execute("c4", { tasks: [{ subject: "b", description: "", blockedBy: [1] }] }, undefined, undefined, ctx);
    expect(staleDep.isError).toBe(true);
    expect(await readFile(path, "utf8")).toBe(beforeDisk);
    expect(widgets.length).toBe(widgetCount);

    const listed = await tools.get("task_list").execute("c5", {}, undefined, undefined, ctx);
    expect(JSON.parse(listed.content[0].text).map((task: { subject: string }) => task.subject)).toEqual(["a"]);

    const recreated = await tools
      .get("task_create")
      .execute("c6", { tasks: [{ subject: "b", description: "" }] }, undefined, undefined, ctx);
    expect(recreated.isError).toBeUndefined();
    expect(JSON.parse(recreated.content[0].text).tasks[0]).toMatchObject({ id: 2, subject: "b" });
    expect(JSON.parse(await readFile(path, "utf8"))).toMatchObject({
      version: 2,
  ...
  it("appends with the existing nextId when any task is still active", async () => {
    ...
    const recreated = await tools.get("task_create").execute("c4", { tasks: [{ subject: "c", description: "" }] }, undefined, undefined, ctx);
    expect(recreated.isError).toBeUndefined();
    expect(JSON.parse(recreated.content[0].text).tasks[0]).toMatchObject({ id: 3, subject: "c" });
...
describe("attempt limits via tools", () => {
  ...
  it("creates with attempt 0 and default maxAttempts 8, and surfaces them in every payload", async () => {
    ...
    expect(JSON.parse(gotten.content[0].text)).toMatchObject({ attempt: 0, maxAttempts: 8 });

    const listed = await tools.get("task_list").execute("c3", {}, undefined, undefined, ctx);
    expect(JSON.parse(listed.content[0].text)[0]).toMatchObject({ attempt: 0, maxAttempts: 8 });
  });
  ...
  it("applies the configured maxAttempts to tool-created tasks", async () => {
    ...
    const created = await tools.get("task_create").execute("c1", { tasks: [{ subject: "W", description: "d" }] }, undefined, undefined, ctx);
    expect(created.isError).toBeUndefined();
    expect(JSON.parse(created.content[0].text).tasks[0]).toMatchObject({ attempt: 0, maxAttempts: 3 });
  });
  ...
  it("excludes maxAttempts from the task_create schema and rejects supplied input", async () => {
    ...
    registerExtension(pi);
    const create = tools.get("task_create").parameters as any;
    expect("maxAttempts" in create.properties.tasks.items.properties).toBe(false);
    ...
    for (const maxAttempts of [2, 0, -1, 1.5, Number.MAX_SAFE_INTEGER]) {
      const bad = await tools
        .get("task_create")
        .execute("bad", { tasks: [{ subject: "W", description: "d", maxAttempts }] }, undefined, undefined, ctx);
    ...
    const listed = await tools.get("task_list").execute("c2", {}, undefined, undefined, ctx);
    expect(JSON.parse(listed.content[0].text)).toEqual([]);
  });
  ...
  it("excludes attempt and maxAttempts from the task_update schema and rejects them at runtime", async () => {
    ...
    for (const patch of [{ id: 1, attempt: 1 }, { id: 1, maxAttempts: 1 }]) {
      const result = await tools.get("task_update").execute("c2", { updates: [patch] }, undefined, undefined, ctx);
      expect(result.isError).toBe(true);
  ...
  it("exposes appendLog and preserves description while appending a timestamped note", async () => {
    ...
    expect(updateItem.properties.description.description).toContain("use appendLog");
    expect(updateItem.properties.appendLog.description).toContain("Execution note appended with a timestamp");
    expect(update.description).not.toContain("appendLog");

    const { ctx } = await freshCtx();
    await tools.get("task_create").execute("c1", { tasks: [{ subject: "Work", description: "original" }] }, undefined, undefined, ctx);
    ...
    expect(JSON.parse(result.content[0].text).updated[0].log.at(-1).message).toBe("Needs another pass");
  ...
  it("returns the updated task and increments attempt immediately on entry", async () => {
    ...
    registerExtension(pi);
    const { ctx } = await freshCtx();
    await tools.get("task_create").execute("c1", { tasks: [{ subject: "Work", description: "d" }] }, undefined, undefined, ctx);
    const updated = await tools.get("task_update").execute("c2", { updates: [{ id: 1, status: "in_progress", appendLog: "note" }] }, undefined, undefined, ctx);
    expect(updated.isError).toBeUndefined();
    expect(JSON.parse(updated.content[0].text).updated[0]).toMatchObject({
      id: 1,
      status: "in_progress",
      attempt: 1 });
    const stored = JSON.parse((await tools.get("task_get").execute("c3", { id: 1 }, undefined, undefined, ctx)).content[0].text);
    expect(stored).toMatchObject({ attempt: 1, maxAttempts: 8, status: "in_progress" });
  });
  ...
  it("rejects exhausted entry with the exact message and preserves persisted state", async () => {
    ...
    await tools
      .get("task_create")
      .execute("c1", { tasks: [{ subject: "Work", description: "d" }] }, undefined, undefined, ctx);
    await tools.get("task_update").execute("c2", { updates: [{ id: 1, status: "in_progress", appendLog: "note" }] }, undefined, undefined, ctx);
    await tools.get("task_update").execute("c3", { updates: [{ id: 1, status: "completed", appendLog: "note" }] }, undefined, undefined, ctx);
    const before = await tools.get("task_get").execute("c4", { id: 1 }, undefined, undefined, ctx);
    const rejected = await tools
      .get("task_update")
      .execute("c5", { updates: [{ id: 1, status: "in_progress", subject: "leaked", appendLog: "note" }] }, undefined, undefined, ctx);
    expect(rejected.isError).toBe(true);
    expect(rejected.content[0].text).toBe("Task #1 has reached the maximum number of attempts (1).");
    const after = await tools.get("task_get").execute("c6", { id: 1 }, undefined, undefined, ctx);
...
describe("renamed prefix guard via tools", () => {
  it("rejects stale prefix on task_create", async () => {
    ...
    const result = await tools
      .get("task_create")
      .execute("c1", { tasks: [{ subject: "W", description: "d", prefix: "api" }] }, undefined, undefined, ctx);
    expect(result.isError).toBe(true);
  ...
  it("rejects stale prefix on task_update", async () => {
    ...
    const result = await tools
      .get("task_update")
      .execute("c2", { updates: [{ id: 1, prefix: "api" }] }, undefined, undefined, ctx);
    expect(result.isError).toBe(true);
    expect(result.content[0].text).toBe("`prefix` was renamed to `assignee`; use `assignee`.");
  });
});
...
describe("batch tool integration", () => {
  it("creates a dependency batch and returns ref mappings plus all tasks", async () => {
    ...
    expect(payload.tasks.map((task: any) => [task.id, task.subject, task.blockedBy])).toEqual([
      [1, "Inspect", []],
      [2, "Implement", [1]],
      [3, "Review", [2]],
    ]);
  ...
  it("updates dependencies declaratively and returns actual post-update tasks", async () => {
    ...
    const result = await tools.get("task_update").execute(
      "update",
      { updates: [{ id: 2, status: "in_progress", appendLog: "note" }, { id: 1, status: "completed", appendLog: "done" }] },
      undefined,
      undefined,
      ctx,
    );
    expect(result.isError).toBeUndefined();
    expect(JSON.parse(result.content[0].text).updated.map((task: any) => [task.id, task.status, task.attempt])).toEqual([
      [1, "completed", 1],
      [2, "in_progress", 1],
    ]);
  });
