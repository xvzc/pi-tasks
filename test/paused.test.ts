...
describe("paused status identity", () => {
  ...
  it("exposes paused through the task_update and task_list status schemas", async () => {
    ...
    const updateStatus = JSON.stringify(
      (tools.get("task_update").parameters as any).properties.updates.items
        .properties.status,
    );
    const listStatus = JSON.stringify(
      (tools.get("task_list").parameters as any).properties.status,
    );
  ...
  it("filters paused tasks through the task_list tool", async () => {
    ...
    const ctx: any = {
      cwd: dir,
      sessionManager: { getSessionId: () => "paused-filter" },
      ui: { setWidget: () => {} },
    };
    await tools
      .get("task_create")
    ...
    await tools
      ...
      .execute(
        "c3",
        { updates: [{ id: 1, status: "paused", appendLog: "broke" }] },
        undefined,
        undefined,
        ctx,
      );
    const filtered = await tools
      .get("task_list")
      .execute("c4", { status: "paused" }, undefined, undefined, ctx);
    expect(filtered.isError).toBeUndefined();
    expect(
      JSON.parse(filtered.content[0].text).map((task: Task) => task.id),
    ).toEqual([1]);
  });
});
...
describe("mandatory appendLog for every transition", () => {
  it("requires a non-empty appendLog on every real status transition", async () => {
    ...
    await expect(
      store.update(pending.id, { status: "in_progress" }),
    ).rejects.toThrow(/appendLog/);
    await expect(
      store.update(pending.id, { status: "in_progress", appendLog: "   " }),
    ).rejects.toThrow(/appendLog/);
    await expect(
      store.update(pending.id, { status: "deleted" }),
    ).rejects.toThrow(/appendLog/);
    ...
    await expect(
      store.update(pending.id, { status: "paused" }),
    ).rejects.toThrow(/appendLog/);
    await expect(
      store.update(pending.id, { status: "completed" }),
    ).rejects.toThrow(/appendLog/);

    vi.setSystemTime(T0 + 10_000);
    await store.update(pending.id, {
      status: "paused",
      appendLog: "pausing for now",
    });
  ...
  it("rejects transitions outside the adjacency map", async () => {
    ...
    await expect(
      store.update(task.id, { status: "paused", appendLog: "note" }),
    ).rejects.toThrow("Task #1 cannot transition from pending to paused.");
    await expect(
      store.update(task.id, { status: "completed", appendLog: "note" }),
    ).rejects.toThrow("Task #1 cannot transition from pending to completed.");
    await store.update(task.id, { status: "in_progress", appendLog: "start" });
    await expect(
      store.update(task.id, { status: "deleted", appendLog: "note" }),
    ).rejects.toThrow("Task #1 cannot transition from in_progress to deleted.");
...
describe("pause timing freeze and resume", () => {
  it("freezes the running attempt into tookMs, clears startedAt, and banks global active time", async () => {
    ...
    const paused = await store.update(task.id, {
      status: "paused",
      appendLog: "boom",
    });
    ...
    expect(formatTaskLine(store.get(task.id) as Task, T0 + 600_000)).toContain(
      "1m 5s",
    );
  });
  ...
  it("resumes via paused -> in_progress with a fresh timer, cleared tookMs, and incremented attempt", async () => {
    ...
    const retried = await store.update(task.id, {
      status: "in_progress",
      appendLog: "note",
    });
...
describe("paused dependencies, reset, and clear", () => {
  it("treats paused dependencies as unsatisfied and rolls back blocked mutations", async () => {
    ...
    await expect(
      store.update(main.id, { status: "in_progress", appendLog: "note" }),
    ...
    const runner = await store.create({
      subject: "runner",
      description: "",
      blockedBy: [other.id],
    });
    ...
    await expect(
      store.update(other.id, {
        status: "in_progress",
        appendLog: "late rework",
      }),
    ).rejects.toThrow(/cannot remain in_progress/);
...
describe("paused rendering", () => {
  it("uses the fixed paused glyph", () => {
    expect(
      statusGlyph(widgetTask({ id: 1, subject: "a", status: "paused" })),
    ).toBe("⏸");
    expect(
      taskRowLabel(widgetTask({ id: 2, subject: "Hi", status: "paused" })),
    ).toBe("⏸ #2 (0/9) Hi");
  });

  it("uses the fixed warning glyph and styles paused subject like pending subject", () => {
    const assignmentConfig = { ...DEFAULT_CONFIG, enableAssignment: true };
  ...
  it("shows N paused in the header only when pauses exist and keeps old headers otherwise", () => {
    expect(buildWidgetLines([widgetTask({ id: 1, subject: "a" })], T0)[0]).toBe(
  ...
  it("never blinks paused glyphs", () => {
    const on = renderWidgetLines(
      [widgetTask({ id: 1, subject: "a", status: "paused" })],
      fakeTheme,
      undefined,
      true,
    );
