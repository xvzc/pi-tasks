...
describe("detail content", () => {
  ...
  it("includes status, id/attempts, assignment, description, blockedBy, timestamps, metadata, and log", () => {
    ...
    expect(joined).toContain("Assignment: @api");
    expect(
      buildTaskDetailLines(task({ id: 4, subject: "None" }), assignmentConfig),
  ...
  it("appends blockedBy ids to pending row subjects only", () => {
    expect(
      taskRowLabel(task({ id: 3, subject: "Hi", blockedBy: [1, 2] })),
    ).toBe("◌ #3 (0/9) Hi → (1, 2)");
    expect(
      taskRowLabel(
        task({
    ...
    expect(
      taskRowLabel(
        task({ id: 3, subject: "Hi", status: "completed", blockedBy: [1, 2] }),
      ),
    ).not.toContain("→");
    expect(
      taskRowLabel(task({ id: 4, subject: "Old", status: "deleted" })),
    ).toContain("Old [deleted]");
  });
});
  ...
  it("stays safe at constrained widths, including wide characters", () => {
    ...
    const wide = createTasksViewer(
      [
        task({
          id: 1,
          subject: "日本語のとても長いサブジェクト".repeat(6),
          description: "説明".repeat(40),
        }),
        task({
          id: 2,
          subject: "b",
...
describe("viewer border theme and keybindings", () => {
  ...
  const borderTheme = {
    fg: (color: string, text: string) =>
      color === "border" ? `\x1b[90m${text}\x1b[39m` : text,
  ...
  it("paints the outer border with the theme border color", () => {
    const done = vi.fn();
    const viewer = createTasksViewer(threeTasks(), {
      done,
      theme: borderTheme,
    });
    const lines = viewer.render(80);
