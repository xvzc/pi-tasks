...
describe("unlimited tasks in the store", () => {
  ...
  it("still rejects an explicit maxAttempts of 0 on create", async () => {
    const store = await freshStore();
    await expect(
      store.create({ subject: "a", description: "", maxAttempts: 0 }),
  ...
  it("round-trips unlimited tasks through the persisted envelope", async () => {
    ...
    expect(
      (await TaskStore.load(store.filePath)).get(created.id),
    ).toMatchObject({ attempt: 1, maxAttempts: 0 });
  });
...
describe("unlimited counter rendering", () => {
  it("renders the remaining-attempts indicator in plain widget lines", () => {
    expect(
      formatTaskLine(task({ id: 1, subject: "Free" }), Date.now()),
    ).toContain("#1 ↻9");
    expect(
      formatTaskLine(task({ id: 1, subject: "Free", maxAttempts: 0 })),
    ).toBe("  ◌ #1 Free");
    expect(
      formatTaskLine(
        task({
        ...
        Date.now(),
        { ...DEFAULT_CONFIG, enableAssignment: true },
      ),
    ).toBe("  ◌ #1 @api Free");
    expect(
      buildWidgetLines([task({ id: 1, subject: "Free", maxAttempts: 0 })])[1],
    ).toBe("  ◌ #1 Free");
  });
  ...
  it("hides the counter in /tasks rows and detail lines", () => {
    expect(taskRowLabel(task({ id: 2, subject: "Free", maxAttempts: 0 }))).toBe(
      "◌ #2 Free",
    );
    expect(
      taskRowLabel(
        task({ id: 2, subject: "Free", blockedBy: [1], maxAttempts: 0 }),
      ),
    ).toBe("◌ #2 Free → (1)");
    const detail = buildTaskDetailLines(
      task({ id: 2, subject: "Free", attempt: 3, maxAttempts: 0 }),
    );
    expect(detail).toContain("Task: #2");
    expect(detail.join("\n")).not.toContain("(3/0)");
    // Limited tasks keep the counter everywhere.
    expect(taskRowLabel(task({ id: 2, subject: "Capped" }))).toBe(
      "◌ #2 (0/9) Capped",
    );
    expect(buildTaskDetailLines(task({ id: 2, subject: "Capped" }))).toContain(
      "Task: #2 (0/9)",
    );
  });
});

describe("fixed glyph characters", () => {
  it("uses the fixed built-in glyph per status", () => {
    ...
    expect(formatTaskLine(task({ id: 1, subject: "a" }), Date.now())).toBe(
      "  ◌ #1 ↻9 a",
    );
    expect(taskRowLabel(task({ id: 1, subject: "a" }))).toBe("◌ #1 (0/9) a");
  });

  it("keeps fixed status colors with fixed glyph characters", () => {
...
describe("unlimited tasks via tools", () => {
  ...
  it("never warns or exhausts unlimited tasks across repeated attempts", async () => {
    const { ctx, notifications } = await unlimitedCtx(
      "unlimited-final-session",
    );
    ...
    const stored = JSON.parse(
      (
        await tools
          .get("task_get")
          .execute("c7", { id: 1 }, undefined, undefined, ctx)
      ).content[0].text,
    );
