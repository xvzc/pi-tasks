...
describe("per-attempt timing transitions", () => {
  it("starts the attempt timer at zero only on a real entry into in_progress", async () => {
    ...
    const started = await store.update(created.id, {
      status: "in_progress",
      appendLog: "note",
    });
    ...
    const stayed = await store.update(created.id, {
      status: "in_progress",
      subject: "renamed",
      appendLog: "note",
    });
  ...
  it("freezes the duration on completion and replaces it after rework", async () => {
    ...
    const done = await store.update(created.id, {
      status: "completed",
      appendLog: "note",
    });
    ...
    expect(
      formatTaskLine(store.get(created.id) as Task, T0 + 600_000),
    ).toContain("1m 5s");
    ...
    const reworked = await store.update(created.id, {
      status: "in_progress",
      appendLog: "note",
    });
    ...
    const redone = await store.update(created.id, {
      status: "completed",
      appendLog: "note",
    });
...
describe("persistence and reload", () => {
  it("persists attempt timing and global timing across reloads", async () => {
    ...
    expect(reloaded.activeTiming().activeSince).toBe(
  ...
  it("rejects invalid persisted timing fields", async () => {
    ...
    await writeFile(
      store.filePath,
      JSON.stringify({ ...base, totalActiveMs: -1 }),
    );
    await expect(TaskStore.load(store.filePath)).rejects.toThrow(
      /totalActiveMs/,
    );
    await writeFile(
      store.filePath,
      JSON.stringify({ ...base, activeSince: "not-a-date" }),
    );
    ...
    await writeFile(
      ...
      JSON.stringify({
        ...base,
        tasks: [{ ...base.tasks[0], startedAt: "bad" }],
      }),
    );
...
describe("timing formatting", () => {
  ...
  it("shows the global total only after timing has started", () => {
    const header = buildWidgetLines([widgetTask({ id: 1, subject: "a" })], T0, {
      totalActiveMs: 0,
    })[0];
    expect(header).toBe("❯ Tasks · 1 total · 0 done");
    const started = buildWidgetLines(
      [widgetTask({ id: 1, subject: "a", attempt: 1 })],
      T0,
      { totalActiveMs: 0 },
    )[0];
  ...
  it("shows zero durations explicitly and hides pending durations", () => {
    expect(formatTaskLine(widgetTask({ id: 1, subject: "a" }), T0)).toBe(
      "  ◌ #1 ↻9 a",
    );
    expect(
      formatTaskLine(
        widgetTask({ id: 1, subject: "a", status: "completed", tookMs: 0 }),
        T0,
      ),
    ).toBe("  ● #1 ↻9 a 0s");
  });
