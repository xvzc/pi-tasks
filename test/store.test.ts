...
describe("persisted assignment schema", () => {
  it("rejects a persisted assignee field as an unknown key in active tasks", async () => {
    const path = await persistedPath({
      version: 2,
      nextId: 2,
      tasks: [persistedTask(1, { assignee: "LegacyOwner" })],
      history: [],
      totalActiveMs: 0,
    });
    await expect(TaskStore.load(path)).rejects.toThrow(
      "Persisted task contains unknown key: assignee.",
    );
  });

  it("rejects a persisted assignee field as an unknown key in history", async () => {
    const path = await persistedPath({
      version: 2,
      nextId: 2,
      tasks: [],
      history: [
        {
          ...
          tasks: [
            persistedTask(1, { status: "completed", assignee: "HistoryOwner" }),
          ],
          totalActiveMs: 0,
        },
      ],
  ...
  it("round-trips assignment using only assignment", async () => {
    const path = await persistedPath({
      version: 2,
      nextId: 2,
      tasks: [
        persistedTask(1, { assignment: { delegate: true, owner: "Owner" } }),
      ],
      history: [],
      totalActiveMs: 0,
    });
    const store = await TaskStore.load(path);
    expect(store.get(1)?.assignment).toEqual({
      delegate: true,
      owner: "Owner",
    });
    await store.save();
    const persisted = JSON.parse(await readFile(path, "utf8")).tasks[0];
    expect(persisted.assignment).toEqual({ delegate: true, owner: "Owner" });
    expect("assignee" in persisted).toBe(false);
    await expect(TaskStore.load(path)).resolves.toBeInstanceOf(TaskStore);
  });
});

describe("assignment mutation boundary", () => {
  it("preserves arbitrary non-empty delegated owners exactly", async () => {
    const store = await freshStore();
    const created = await store.create({
      subject: "Opaque owner",
      description: "",
    ...
    const updated = await store.update(created.id, {
      assignment: { delegate: true, owner: "plugin:missing-at-runtime" },
    });
    expect(updated.assignment).toEqual({
      delegate: true,
      owner: "plugin:missing-at-runtime",
    });
  });

  it("rejects structurally invalid delegated owners atomically", async () => {
    const store = await freshStore();
    await expect(
      store.createMany([
        {
          subject: "Valid",
          description: "",
          assignment: { delegate: true, owner: "any-owner" },
        },
        {
          subject: "Blank",
          description: "",
          assignment: { delegate: true, owner: "   " },
        },
      ]),
    ).rejects.toThrow(/delegate: true, owner: string/);
    expect(store.list()).toEqual([]);

    await store.createMany([
      { subject: "One", description: "" },
      { subject: "Two", description: "" },
    ]);
    const before = store.list();
    await expect(
      store.updateMany([
        { id: 1, assignment: { delegate: true, owner: "opaque" } },
        { id: 2, assignment: { delegate: true, owner: "" } },
      ]),
    ).rejects.toThrow(/delegate: true, owner: string/);
...
describe("create/get/list and deleted tombstones", () => {
  ...
  it("uses independent temporary files for concurrent saves and cleans them up", async () => {
    ...
    expect(
      (await readdir(join(store.filePath, ".."))).filter((name) =>
        name.endsWith(".tmp"),
      ),
    ).toEqual([]);
  });
  ...
  it("rejects invalid create input", async () => {
    ...
    await expect(
      store.create({ subject: "x", description: "", blockedBy: [42] }),
  ...
  it("requires description on create", async () => {
    const store = await freshStore();
    await expect(
      store.create({ subject: "x" } as unknown as {
        subject: string;
        description: string;
      }),
    ).rejects.toThrow(TaskError);
  ...
  it("rejects invalid ids", async () => {
    const store = await freshStore();
    await expect(store.update(0, { subject: "x" })).rejects.toThrow(
      /Invalid task id/,
    );
    await expect(
      store.update(-3, { status: "deleted", appendLog: "note" }),
    ).rejects.toThrow(/Invalid task id/);
    expect(() => store.get(Number.MAX_SAFE_INTEGER + 1)).toThrow(
    ...
    await expect(
      store.create({
        subject: "x",
        description: "",
        blockedBy: [Number.MAX_SAFE_INTEGER + 1],
      }),
    ).rejects.toThrow(/positive safe integer/);
    await expect(store.update(7, { subject: "x" })).rejects.toThrow(
      /does not exist/,
    );
    await expect(
      store.update(7, { status: "deleted", appendLog: "note" }),
    ).rejects.toThrow(/does not exist/);
  });
});
...
describe("persisted store validation", () => {
  it("accepts missing/version-1 files but rejects missing and unsupported versions", async () => {
    ...
    await expect(TaskStore.load(missing.filePath)).resolves.toBeInstanceOf(
      TaskStore,
    );
    await expect(
      TaskStore.load(await persistedPath({ nextId: 1, tasks: [] })),
    ).rejects.toThrow(/version/);
    await expect(
      TaskStore.load(await persistedPath({ version: 1, nextId: 1, tasks: [] })),
    ...
    await expect(
      TaskStore.load(await persistedPath({ version: 3, nextId: 1, tasks: [] })),
    ).rejects.toThrow(/version/);
  });

  it("rejects invalid or stale nextId values", async () => {
    await expect(
      TaskStore.load(await persistedPath({ version: 1, nextId: 0, tasks: [] })),
  ...
  it("rejects unknown version-1 envelope and task keys while allowing free-form metadata", async () => {
    await expect(
      TaskStore.load(
        await persistedPath({ version: 1, nextId: 1, tasks: [], extra: true }),
      ),
    ...
    await expect(
      TaskStore.load(
        await persistedPath({
          version: 1,
          nextId: 2,
          tasks: [persistedTask(1, { extra: true })],
        }),
      ),
    ).rejects.toThrow(/unknown key: extra/);
    await expect(
      TaskStore.load(
        await persistedPath({
          version: 1,
          nextId: 2,
          tasks: [persistedTask(1, { metadata: { extra: true } })],
        }),
      ),
    ).resolves.toBeInstanceOf(TaskStore);
  });
  ...
  it("validates version-2 completed-cycle history", async () => {
    const validCycle = {
      archivedAt: timestamp,
      tasks: [
        persistedTask(1, { status: "completed" }),
        persistedTask(2, { status: "deleted" }),
      ],
      totalActiveMs: 42,
    };
    const path = await persistedPath({
    ...
    expect(store.listHistory()).toMatchObject([
      {
        archivedAt: timestamp,
        totalActiveMs: 42,
        tasks: [{ id: 1 }, { id: 2, status: "deleted" }],
      },
    ]);
  ...
  it("allocates the last ID with a representable counter, then rejects exhausted creation", async () => {
    ...
    expect(JSON.parse(await readFile(path, "utf8")).nextId).toBe(
      Number.MAX_SAFE_INTEGER,
    );

    const before = await readFile(path, "utf8");
    await expect(
      store.create({ subject: "exhausted", description: "" }),
    ...
    await expect(
      TaskStore.load(
        await persistedPath({
          version: 1,
          nextId: 2,
          tasks: [persistedTask(1, { createdAt: "not-a-date" })],
        }),
      ),
    ...
    await expect(
      TaskStore.load(
        await persistedPath({
          version: 1,
          nextId: 2,
          tasks: [persistedTask(1), persistedTask(1)],
        }),
      ),
  ...
  it.each([
    ...
    [
      "cycle",
      [
        persistedTask(1, { blockedBy: [2] }),
        persistedTask(2, { blockedBy: [1] }),
      ],
    ],
    [
      ...
      [
        persistedTask(1),
        persistedTask(2, { status: "in_progress", blockedBy: [1] }),
      ],
    ],
  ])("rejects an invalid dependency graph: %s", async (_name, tasks) => {
    await expect(
      TaskStore.load(await persistedPath({ version: 1, nextId: 3, tasks })),
    ).rejects.toThrow(TaskError);
  });
});
...
describe("timestamps and patch semantics", () => {
  ...
  it("shallow-merges metadata", async () => {
    ...
    const task = await store.create({
      subject: "a",
      description: "",
      metadata: { keep: 1, over: "old" },
    });
    const updated = await store.update(task.id, {
      metadata: { over: "new", added: true },
    });
  ...
  it("appends timestamped log notes without changing the description", async () => {
    ...
    expect((await TaskStore.load(store.filePath)).get(task.id)?.log).toEqual(
      updated.log,
    );
    await expect(store.update(task.id, { appendLog: "   " })).rejects.toThrow(
      /non-empty string/,
    );
  });

  it("removes assignment on null", async () => {
    const store = await freshStore();
    const task = await store.create(
      {
        subject: "a",
    ...
    expect(task.assignment).toEqual({ delegate: true, owner: "api" });
    const updated = await store.update(task.id, { assignment: null });
    expect("assignment" in updated).toBe(false);
  });
  ...
  it("reopens completed tasks via the rework route", async () => {
    ...
    const reopened = await store.update(task.id, {
      status: "in_progress",
      appendLog: "rework",
    });
    ...
    await expect(
      store.update(task.id, { status: "pending", appendLog: "note" }),
    ).rejects.toThrow("Task #1 cannot transition from in_progress to pending.");
  });
  ...
  it("rolls back create, nextId, and deleted transition when persistence fails", async () => {
    ...
    await expect(
      store.create({ subject: "not saved", description: "" }),
    ).rejects.toThrow(/simulated write failure/);
    expect(store.list()).toEqual(before);
    await expect(
      store.update(1, { status: "deleted", appendLog: "note" }),
    ).rejects.toThrow(/simulated write failure/);
    expect(store.list()).toEqual(before);
    expect(await readFile(original.filePath, "utf8")).toBe(beforeDisk);

    failWrites = false;
    expect((await store.create({ subject: "saved", description: "" })).id).toBe(
      2,
    );
  });
});
...
describe("dependencies", () => {
  it("refuses unknown references and self-reference", async () => {
    const store = await freshStore();
    const task = await store.create({ subject: "a", description: "" });
    await expect(store.update(task.id, { blockedBy: [999] })).rejects.toThrow(
      /does not exist/,
    );
    await expect(
      store.update(task.id, { blockedBy: [task.id] }),
    ).rejects.toThrow(/itself/);
    await expect(
      store.create({
        subject: "b",
        description: "",
        blockedBy: [task.id, 999],
      }),
    ).rejects.toThrow(/does not exist/);
  });
  ...
  it("refuses cycles", async () => {
    ...
    await expect(store.update(a.id, { blockedBy: [c.id] })).rejects.toThrow(
      /cycle/,
    );
    await expect(store.update(b.id, { blockedBy: [b.id] })).rejects.toThrow(
      /itself/,
    );
    // Direct two-node cycle.
    await expect(store.update(a.id, { blockedBy: [b.id] })).rejects.toThrow(
      /cycle/,
    );
  });
  ...
  it("gates entering in_progress on completed dependencies", async () => {
    ...
    await expect(
      store.update(task.id, { status: "in_progress", appendLog: "note" }),
    ...
    const started = await store.update(task.id, {
      status: "in_progress",
      appendLog: "note",
    });
  ...
  it("rejects replacing dependencies of an in-progress task with incomplete tasks", async () => {
    ...
    await expect(
      store.update(task.id, { blockedBy: [pending.id] }),
    ).rejects.toThrow(/Task #3 cannot remain in_progress.*#2/);
  ...
  it("rejects reopening a dependency required by an in-progress task", async () => {
    ...
    await expect(
      store.update(dep.id, { status: "in_progress", appendLog: "rework" }),
    ).rejects.toThrow(/Task #2 cannot remain in_progress.*#1/);
  ...
  it("refuses a referenced deleted transition but allows an atomic dependent tombstone", async () => {
    ...
    await expect(
      store.update(dep.id, { status: "deleted", appendLog: "note" }),
    ).rejects.toThrow(/#2/);
    ...
    expect(deleted.map((entry) => entry.status)).toEqual([
      "deleted",
      "deleted",
    ]);
  ...
  it("makes deleted terminal while allowing later log and metadata updates", async () => {
    ...
    await store.update(task.id, {
      status: "deleted",
      appendLog: "removed from scope",
    });
    await expect(
      store.update(task.id, { status: "pending", appendLog: "note" }),
    ).rejects.toThrow(/cannot transition/);

    const annotated = await store.update(task.id, {
    ...
    expect(annotated.log.map((entry) => entry.message)).toEqual([
      "removed from scope",
      "confirmed",
    ]);
  });
...
describe("attempt limits", () => {
  ...
  it("accepts large maxAttempts with no upper cap, including Number.MAX_SAFE_INTEGER", async () => {
    ...
    for (const maxAttempts of [17, 100, Number.MAX_SAFE_INTEGER]) {
      await expect(
        store.create({ subject: "a", description: "", maxAttempts }),
  ...
  it("accepts a large configured maxAttempts with no upper cap", async () => {
    const store = await freshStore();
    await expect(
      store.create({ subject: "a", description: "" }, 17),
    ).resolves.toMatchObject({
      maxAttempts: 17,
    });
    await expect(
      store.create({ subject: "b", description: "" }, Number.MAX_SAFE_INTEGER),
    ).resolves.toMatchObject({
  ...
  it("rejects unsafe or non-integer configured maxAttempts", async () => {
    ...
    await expect(
      store.create({ subject: "a", description: "" }, 1.5),
    ).rejects.toThrow(/maxAttempts/);
    expect(store.list()).toEqual([]);
  });
  ...
  ])("rejects invalid maxAttempts on create: %s", async (maxAttempts) => {
    const store = await freshStore();
    await expect(
      store.create({
        subject: "a",
  ...
  it("rejects non-numeric maxAttempts on create", async () => {
    ...
    await expect(
      store.create({
        subject: "a",
        description: "",
        maxAttempts: "3" as unknown as number,
      }),
    ).rejects.toThrow(/maxAttempts/);
  });
  ...
  it("keeps per-task limits independent", async () => {
    ...
    await expect(
      store.update(a.id, { status: "completed", appendLog: "note" }),
    ).resolves.toMatchObject({ attempt: 1 });
    await expect(
      store.update(a.id, { status: "in_progress", appendLog: "note" }),
    ).rejects.toThrow(
      "Task #1 has reached the maximum number of attempts (1).",
    );
    const started = await store.update(b.id, {
      status: "in_progress",
      appendLog: "note",
    });
  ...
  it("follows the entry transition matrix", async () => {
    ...
    await store.update(task.id, { status: "paused", appendLog: "note" });
    await expect(
      store.update(task.id, { status: "completed", appendLog: "note" }),
    ).rejects.toThrow("Task #1 cannot transition from paused to completed.");
    // Non-status patch on in_progress does not increment.
    await store.update(task.id, { status: "in_progress", appendLog: "note" });
    expect((await store.update(task.id, { subject: "renamed" })).attempt).toBe(
      4,
    );
  ...
  it("excludes attempt and maxAttempts from task_update with no workaround", async () => {
    ...
    const task = await store.create({ subject: "a", description: "" });
    await expect(
      store.update(task.id, { attempt: 5 } as unknown as Record<
        string,
        never
      > as never),
    ).rejects.toThrow(/attempt/);
    await expect(
      store.update(task.id, { maxAttempts: 5 } as unknown as Record<
        string,
        never
      > as never),
    ).rejects.toThrow(/maxAttempts/);
  ...
  it("rejects persisted tasks with missing or invalid attempt/maxAttempts", async () => {
    ...
    await expect(
      TaskStore.load(
        await persistedPath({ version: 1, nextId: 2, tasks: [missingAttempt] }),
      ),
    ).rejects.toThrow(/attempt/);
    ...
    await expect(
      TaskStore.load(
        await persistedPath({ version: 1, nextId: 2, tasks: [missingMax] }),
      ),
    ).rejects.toThrow(/maxAttempts/);
      ...
      await expect(
        TaskStore.load(
          await persistedPath({
            version: 1,
            nextId: 2,
            tasks: [persistedTask(1, { maxAttempts })],
          }),
        ),
      ).rejects.toThrow(/maxAttempts/);
    }
    await expect(
      TaskStore.load(
        await persistedPath({
          version: 1,
          nextId: 2,
          tasks: [persistedTask(1, { attempt: 10 })],
        }),
      ),
    ).rejects.toThrow(/attempt/);
  });
  ...
  it("accepts persisted maxAttempts 0 as the unlimited sentinel without an attempt ceiling", async () => {
    const store = await TaskStore.load(
      await persistedPath({
        version: 1,
        nextId: 2,
        tasks: [persistedTask(1, { maxAttempts: 0, attempt: 12 })],
      }),
    );
  ...
  it("round-trips attempt counters through the persisted envelope", async () => {
    ...
    expect((await TaskStore.load(store.filePath)).get(task.id)).toMatchObject({
      attempt: 1,
      maxAttempts: 2,
    });
  });
  ...
  it("rejects color on create and update", async () => {
    ...
    await expect(
      store.create({
        ...
        description: "",
        color: "red",
      } as unknown as { subject: string; description: string }),
...
describe("renamed prefix guard", () => {
  it("rejects stale prefix on create", async () => {
    ...
    ).rejects.toThrow("`prefix` is no longer supported.");
    expect(store.list()).toEqual([]);
  });
  ...
  it("rejects stale prefix on update", async () => {
    ...
    const task = await store.create({ subject: "a", description: "" });
    await expect(
      store.update(task.id, { prefix: "api" } as unknown as {
        subject: string;
      }),
    ).rejects.toThrow("`prefix` is no longer supported.");
    ...
    await expect(
      TaskStore.load(
        await persistedPath({
          version: 1,
          nextId: 2,
          tasks: [persistedTask(1, { prefix: "api" })],
        }),
      ),
