...
describe("review graph semantics", () => {
  it("creates reviewOfRefs in writer-before-reviewer order and gates only the reviewer", async () => {
    ...
    await expect(
      store.update(reviewer.id, { status: "in_progress", appendLog: "note" }),
    ).rejects.toThrow(
      "Task #2 cannot remain in_progress: dependencies not completed: #1.",
    );
    await expect(
      store.update(writer.id, { status: "in_progress", appendLog: "note" }),
  ...
  it("rejects numeric self-review with the dependency self-reference error", async () => {
    const store = await freshStore();
    await expect(
      store.create({ subject: "Self review", description: "", reviewOf: [1] }),
    ).rejects.toThrow("Task #1 cannot depend on itself.");
  });

  it("reports combined in-progress reviewOf structure errors before eligibility errors", async () => {
    const store = await freshStore();
    const reviewer = await store.create({
      subject: "Reviewer",
      description: "",
    });
  ...
  it("enforces one active reviewer and makes a deleted reviewer inert", async () => {
    ...
    ).rejects.toThrow("Task #1 is already reviewed by #2.");
    await expect(
      store.update(writer.id, { status: "deleted", appendLog: "note" }),
    ).rejects.toThrow("Task #1 cannot be deleted: referenced by #2.");
  ...
  it("uses direct review completion for acceptance even when the reviewer awaits another review", () => {
    ...
    expect(taskRowLabel(reviewer, tasks)).toContain("○ #2");
  });
...
describe("transitionEligibility", () => {
  it("is pure and preserves lifecycle error order", () => {
    ...
    const overflowCurrent = task({
      id: 2,
      attempt: Number.MAX_SAFE_INTEGER,
      maxAttempts: 0,
    });
    const overflowProposed = task({
      ...overflowCurrent,
      status: "in_progress",
      reviewOf: [2],
    });
    expect(
      transitionEligibility(
        overflowCurrent,
        overflowProposed,
        new Map([[2, overflowProposed]]),
        "note",
      ),
    ).toEqual({
      allowed: false,
      error:
        "Task #2 cannot enter in_progress: attempt counter has reached Number.MAX_SAFE_INTEGER.",
    });

    const missing = task({ id: 2, reviewOf: [999] });
    expect(
      transitionEligibility(task({ id: 2 }), missing, new Map([[2, missing]])),
    ).toEqual({
      allowed: false,
    ...
    expect(
      transitionEligibility(task({ id: 2 }), self, new Map([[2, self]])),
    ).toEqual({
      allowed: false,
      error: "Task #2 cannot depend on itself.",
    });

    const blockedProposed = task({
      id: 2,
      status: "in_progress",
      blockedBy: [1],
      reviewOf: [3, 1],
    ...
    expect(
      ...
      allowed: false,
      error:
        "Task #2 cannot remain in_progress: dependencies not completed: #1, #3.",
    });
  });
});
