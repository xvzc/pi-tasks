...
/** Assignment display enabled: every `@assignment` assertion below pins this config. */
...
describe("plain rendering", () => {
  it("shows numeric id, remaining attempts, optional @assignment, and subject", () => {
    ...
    ).toBe(`  ◌ #3 ↻9 @api Ship it`);
    expect(
      formatTaskLine(
        task({ id: 3, subject: "Ship it", attempt: 2, maxAttempts: 3 }),
        FIXED_NOW,
      ),
    ).toBe(`  ■ #3 ↻1 Ship it`);
  });
  ...
  it("shows blockedBy ids after the subject only while pending", () => {
    expect(
      formatTaskLine(
        task({ id: 4, subject: "Wait", blockedBy: [1, 3] }),
        FIXED_NOW,
      ),
    ).toBe("  ◌ #4 ↻9 Wait → (1, 3)");
    expect(
  ...
  it("uses status-specific default glyphs", () => {
    ...
    expect(
      statusGlyph(task({ id: 1, subject: "a", status: "in_progress" })),
    ).toBe("◌");
    expect(
      statusGlyph(task({ id: 1, subject: "a", status: "completed" })),
    ).toBe("●");
    ...
    const lines = buildWidgetLines(
      [
        task({
          id: 3,
          subject: "c",
          assignment: { delegate: true, owner: "x" },
        }),
  ...
  it("keeps deleted tombstones visible in plain lines and header counts", () => {
    const lines = buildWidgetLines(
      [task({ id: 1, subject: "obsolete", status: "deleted", tookMs: 500 })],
      FIXED_NOW,
    );
  ...
  it("hides total time until a task starts and keeps zero after a start", () => {
    expect(
      buildWidgetLines([task({ id: 1, subject: "a" })], FIXED_NOW)[0],
    ).toBe("❯ Tasks · 1 total · 0 done");
    expect(
      buildWidgetLines(
    ...
    expect(
      buildWidgetLines(
        [task({ id: 1, subject: "a", status: "completed", tookMs: 5_000 })],
        FIXED_NOW,
      )[0],
...
describe("themed rendering", () => {
  ...
  it("renders the header title in accent and the stats tail in dim", () => {
    ...
    expect(active[0]).toBe(
      `<accent>❯ *Tasks*</><dim> · 1 total · 0 done · ${FIXED_ELAPSED}</>`,
    );
    expect(active[0]).toContain("<accent>❯ *Tasks*</>");
    expect(active[0]).toContain(
      `<dim> · 1 total · 0 done · ${FIXED_ELAPSED}</>`,
    );
  ...
  it("renders in-progress subjects/assignments as text+bold with success glyph and dim elapsed", () => {
    const pending = renderWidgetLines(
      [task({ id: 1, subject: "p" })],
      fakeTheme,
    );
  ...
  it("renders paused glyph in warning while paused subject matches pending subject styling", () => {
    ...
    const pendingAssignment = renderWidgetLines(
      [
        task({
          id: 3,
          subject: "same",
    ...
    expect(pendingAssignment[1]).toContain("<text>@api</>");
...
describe("width-aware rendering", () => {
  ...
  const longAssignment = `assignment-${"x".repeat(120)}`;
  ...
  function mixedTasks(): Task[] {
    return [
      task({
        id: 1,
        subject: longSubject,
        assignment: { delegate: true, owner: longAssignment },
      }),
      task({
        id: 2,
        subject: longSubject,
  ...
  it("fits long subjects and assignments within 80 columns", () => {
  ...
  it.each([10, 5, 2, 1])(
    "fits every line within a very narrow width of %i columns",
    (width) => {
      for (const theme of [fakeTheme, ansiTheme]) {
  ...
  it("passes non-positive widths through without throwing", () => {
    for (const width of [0, -3, Number.NaN]) {
      expect(() =>
        renderWidgetLines(mixedTasks(), fakeTheme, width),
      ).not.toThrow();
    }
  });
});
  ...
  it("renders themed ANSI output within width via the callback", () => {
    const ansiTheme: ThemeLike = {
      fg: (color: string, text: string) =>
        `\x1b[${color.length}m${text}\x1b[39m`,
      bold: (text: string) => `\x1b[1m${text}\x1b[22m`,
      strikethrough: (text: string) => `\x1b[9m${text}\x1b[29m`,
    };
    const component = createTaskWidget(
...
describe("in-progress blink", () => {
  ...
  it("updates the mounted snapshot and timer lifecycle without replacement", () => {
    ...
    try {
      ...
      component.update([
        task({ id: 1, subject: "running", status: "in_progress" }),
      ]);
...
describe("remaining-attempts indicator rendering", () => {
  it("shows the remaining-attempt count immediately after the id in plain lines", () => {
    ...
    expect(
      formatTaskLine(
        task({ id: 7, subject: "s", attempt: 2, maxAttempts: 5 }),
        FIXED_NOW,
      ),
    ).toBe(`  ■ #7 ↻3 s`);
    const lines = buildWidgetLines(
  ...
  it("shows the dim remaining-attempt count after the id for every status without changing glyph/subject styles", () => {
    const pending = renderWidgetLines(
      [task({ id: 1, subject: "a", attempt: 0, maxAttempts: 9 })],
      fakeTheme,
    );
  ...
  it("keeps the remaining-attempt count next to the id when an assignment is present", () => {
    expect(
      formatTaskLine(
        task({
          id: 1,
  ...
  it("is presentation only and still truncates within width", () => {
    ...
    const lines = renderWidgetLines(
      [task({ id: 1, subject: longSubject, attempt: 8, maxAttempts: 9 })],
      fakeTheme,
      40,
    );
    for (const line of lines)
...
describe("elapsed duration", () => {
  it("formats nonzero day/hour/minute/second components in order and omits zeros", () => {
    expect(formatElapsedDuration("2026-01-01T00:00:00.000Z", FIXED_NOW)).toBe(
      "1d 1h 1m 1s",
    );
    expect(formatElapsedDuration("2026-01-02T01:00:31.000Z", FIXED_NOW)).toBe(
      "30s",
    );
    expect(formatElapsedDuration("2026-01-02T01:00:01.000Z", FIXED_NOW)).toBe(
      "1m",
    );
    expect(formatElapsedDuration("2026-01-02T00:01:01.000Z", FIXED_NOW)).toBe(
      "1h",
    );
    expect(formatElapsedDuration("2026-01-01T01:01:01.000Z", FIXED_NOW)).toBe(
      "1d",
    );
  ...
  it("shows 0s immediately for a newly in_progress task in plain and themed lines", () => {
    const started = task({
      ...
      subject: "s",
      status: "in_progress",
      startedAt: "2026-01-02T01:01:01.000Z",
    });
  ...
  it("keeps pure rendering deterministic for a supplied current time", () => {
    ...
    expect(buildWidgetLines([t], FIXED_NOW)).toEqual(
      buildWidgetLines([t], FIXED_NOW),
    );
    expect(
      renderWidgetLines([t], fakeTheme, undefined, true, FIXED_NOW),
    ).toEqual(renderWidgetLines([t], fakeTheme, undefined, true, FIXED_NOW));
  });
  ...
  it(`requests redraws every ${ELAPSED_INTERVAL_MS} ms only while a task is in progress`, () => {
    ...
    try {
      ...
      component.dispose();
      vi.advanceTimersByTime(5000);
      const callsAfterDispose = (tui.requestRender as ReturnType<typeof vi.fn>)
        .mock.calls.length;

      const idleTui = { terminal: { columns: 80 }, requestRender: vi.fn() };
      const idle = createTaskWidget(
        [task({ id: 1, subject: "a" })],
        idleTui,
        fakeTheme,
        FIXED_NOW,
      ...
      idle.dispose();
      expect(
        (tui.requestRender as ReturnType<typeof vi.fn>).mock.calls.length,
...
describe("remaining-attempts indicator mappings", () => {
  it("counts every attempt including the first", () => {
    ...
    expect(
      remainingRetries(
        task({ id: 1, subject: "x", attempt: 0, maxAttempts: 8 }),
      ),
    ).toBe(" ↻8");
    expect(
      remainingRetries(
        task({ id: 1, subject: "x", attempt: 1, maxAttempts: 8 }),
      ),
    ).toBe(" ↻7");
    expect(
      remainingRetries(
        task({ id: 1, subject: "x", attempt: 2, maxAttempts: 8 }),
      ),
    ).toBe(" ↻6");
    expect(
      remainingRetries(
        task({ id: 1, subject: "x", attempt: 8, maxAttempts: 8 }),
      ),
    ).toBe(" ↻0");
    expect(
      remainingRetries(
        task({ id: 1, subject: "x", attempt: 2, maxAttempts: 3 }),
      ),
    ).toBe(" ↻1");
    expect(
      remainingRetries(
        task({ id: 1, subject: "x", attempt: 1, maxAttempts: 2 }),
      ),
    ).toBe(" ↻1");
  });
  ...
  it("maps exhausted to ↻0", () => {
    expect(
      remainingRetries(
        task({ id: 1, subject: "x", attempt: 9, maxAttempts: 9 }),
      ),
    ).toBe(" ↻0");
    expect(
      remainingRetries(
        task({ id: 1, subject: "x", attempt: 0, maxAttempts: 9 }),
      ),
    ).toBe(" ↻9");
  });
  ...
  it("omits the indicator for unlimited tasks", () => {
    expect(
      remainingRetries(task({ id: 1, subject: "x", maxAttempts: 0 })),
    ).toBe("");
    expect(
      remainingRetries(
        task({ id: 1, subject: "x", attempt: 7, maxAttempts: 0 }),
      ),
    ).toBe("");
    expect(
      formatTaskLine(
  ...
  it("clamps over-consumed attempts to ↻0", () => {
    expect(
      remainingRetries(
        task({ id: 1, subject: "x", attempt: 5, maxAttempts: 3 }),
      ),
    ).toBe(" ↻0");
  });
  ...
  it("renders exact counts for large caps without quantization", () => {
    expect(
      remainingRetries(
        task({ id: 1, subject: "x", attempt: 0, maxAttempts: 30 }),
      ),
    ).toBe(" ↻30");
    expect(
      remainingRetries(
        task({ id: 1, subject: "x", attempt: 10, maxAttempts: 25 }),
      ),
    ).toBe(" ↻15");
    expect(
      remainingRetries(
        task({ id: 1, subject: "x", attempt: 29, maxAttempts: 30 }),
      ),
    ).toBe(" ↻1");
    expect(
      remainingRetries(
        task({ id: 1, subject: "x", attempt: 12, maxAttempts: 24 }),
      ),
    ).toBe(" ↻12");
  });
...
describe("id column alignment", () => {
  function alignedTasks(): Task[] {
    return [
      task({
        id: 9,
        subject: "Sample task 9",
        assignment: { delegate: true, owner: "unassigned" },
        maxAttempts: 8,
      }),
      task({
        id: 10,
        subject: "Sample task 10",
        assignment: { delegate: true, owner: "unassigned" },
        maxAttempts: 8,
      }),
      task({
        id: 11,
        subject: "Sample task 11",
        assignment: { delegate: true, owner: "unassigned" },
        maxAttempts: 8,
      }),
    ];
  ...
  it("preserves standalone formatTaskLine output by default with an opt-in width", () => {
    ...
    expect(formatTaskLine(single, FIXED_NOW, assignmentConfig)).toBe(
      "  ◌ #9 ↻8 @unassigned Sample task 9",
    );
    expect(
      formatTaskLine(single, FIXED_NOW, assignmentConfig, "#10".length),
    ).toBe("  ◌ #9  ↻8 @unassigned Sample task 9");
  });
...
describe("assignment column alignment", () => {
  it("left-aligns @assignment labels, reserves mixed-set gaps, and treats unassigned as a value", () => {
    const tasks = [
      task({
        id: 1,
    ...
    expect(assignmentColumnWidth(tasks, assignmentConfig)).toBe(
      "@long-agent".length,
    );
    const lines = buildWidgetLines(
      tasks,
    ...
    const subjectPositions = lines.map((line, index) =>
      line.indexOf(tasks[index].subject),
    );
    expect(new Set(subjectPositions).size).toBe(1);
  });

  it("adds no assignment column when every task is truly unassigned", () => {
    const tasks = [
      task({ id: 1, subject: "a" }),
      task({ id: 2, subject: "b" }),
    ];
    expect(assignmentColumnWidth(tasks)).toBe(0);
  ...
  it("styles each entire padded assignment field like its status subject", () => {
    const tasks = [
      task({
        id: 1,
        subject: "a",
  ...
  it("uses visible width for wide assignment labels and keeps standalone output unpadded", () => {
    const tasks = [
      task({
        id: 1,
        subject: "a",
        assignment: { delegate: true, owner: "界" },
      }),
      task({
        id: 2,
        subject: "b",
    ...
    expect(assignmentColumnWidth(tasks, assignmentConfig)).toBe(4);
    const lines = buildWidgetLines(
      tasks,
      FIXED_NOW,
      undefined,
    ...
    expect(formatTaskLine(tasks[0], FIXED_NOW, assignmentConfig)).toBe(
      "  ◌ #1 ↻9 @界 a",
    );
  });
});
...
describe("remaining-attempts column alignment", () => {
  ...
  it("aligns themed remaining-attempts columns without changing glyph styling", () => {
    ...
    const widths = lines
      .slice(1)
      .map((line) => visibleWidth(line.slice(0, line.indexOf("<text>"))));
  ...
  it("preserves standalone output with opt-in remaining-attempts width", () => {
    ...
    expect(
      formatTaskLine(single, FIXED_NOW, undefined, undefined, "↻101".length),
    ).toBe("  ◌ #1 ↻8   a");
  });
});
...
describe("header format", () => {
  ...
  it("renders pending headers without duration in plain and themed output", () => {
    ...
    expect(buildWidgetLines(tasks, FIXED_NOW)[0]).toBe(
      "❯ Tasks · 2 total · 0 done",
    );
    expect(
      renderWidgetLines(tasks, fakeTheme, undefined, true, FIXED_NOW)[0],
    ).toBe("<accent>❯ *Tasks*</><dim> · 2 total · 0 done</>");
  });
  ...
  it("uses literal Tasks for a singular total", () => {
    ...
    ).toBe("<accent>❯ *Tasks*</><dim> · 1 total · 0 done</>");
  });
  ...
  it("appends the active duration last once started", () => {
    ...
    expect(
      buildWidgetLines(tasks, FIXED_NOW, { totalActiveMs: SIX_M_37_S })[0],
    ).toBe("❯ Tasks · 3 total · 1 done · 6m 37s");
    expect(
      renderWidgetLines(tasks, fakeTheme, undefined, true, FIXED_NOW, {
        totalActiveMs: SIX_M_37_S,
      })[0],
    ).toBe("<accent>❯ *Tasks*</><dim> · 3 total · 1 done · 6m 37s</>");
    ...
    expect(
      buildWidgetLines(running, FIXED_NOW, {
        totalActiveMs: 60_000,
        activeSince: runningSince,
      })[0],
  ...
  it("appends paused only when pauses exist, keeping duration last", () => {
    ...
    expect(
      buildWidgetLines(withoutPause, FIXED_NOW, {
        totalActiveMs: SIX_M_37_S,
      })[0],
    ).toBe("❯ Tasks · 2 total · 1 done · 6m 37s");
    const withPause = [
      task({
        id: 1,
        subject: "a",
    ...
    expect(
      buildWidgetLines(withPause, FIXED_NOW, { totalActiveMs: SIX_M_37_S })[0],
    ).toBe("❯ Tasks · 3 total · 1 done · 1 paused · 6m 37s");
    expect(
      renderWidgetLines(withPause, fakeTheme, undefined, true, FIXED_NOW, {
        totalActiveMs: SIX_M_37_S,
      })[0],
    ).toBe(
  ...
  it("bolds only Tasks and keeps duration dim", () => {
    ...
    expect(pausedOnly).toBe(
      "<accent>❯ *Tasks*</><dim> · 3 total · 1 done · 1 paused</>",
    );
